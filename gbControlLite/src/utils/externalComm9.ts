...
export class EComm {
    ...
    public sendAudio(data: Buffer, robotID: number): number {
        const prefix = Buffer.from([(0b10000000 + robotID)]);
        return this.send(Buffer.concat([prefix, data]));
    }

    public sendVideo(data: Buffer, robotID: number): number {
        const prefix = Buffer.from([(0b10100000 + robotID)]);
        return this.send(Buffer.concat([prefix, data]));
    }
