...
<script>
import "@/krata/styles/global.scss";
import { Map } from "mapbox-gl";
import VDeckgl from "@/krata/components/base/v-deckgl.vue";
...
import Feature, { FEATURE_TYPES } from "@/krata/models/Feature.model";
...
export default {
	...
	setup() {
		...
		const ACCESS_TOKEN = import.meta.env.VUE_APP_MAPBOX_TOKEN;
		...
		watch(focusedFeature, (feature) => {
			if (feature) {
				...
				try {
					...
					const { latitude, longitude, bearing, pitch } = boundedViewState;
					let { zoom } = boundedViewState;
					zoom =
		...
		const updateViewStateWithBoundingBox = (bbox) => {
			if (bbox) {
				...
				try {
					...
					const { latitude, longitude, zoom, bearing, pitch } = boundedViewState;
					updateViewState({
						latitude,
						longitude,
