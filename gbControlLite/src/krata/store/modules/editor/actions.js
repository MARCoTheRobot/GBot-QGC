...
import Feature, { CASCADABLE_PROPERTIES } from "@/krata/models/Feature.model.js";
...
import { PROJECT_ACTIONS } from "@/krata/utils/constants";
import { generateEditorScreenshot } from "@/krata/utils/Screenshot";
import { fileReader } from "@/krata/utils/FileUtils";
...
const getLastItemIndex = (features) => {
  ...
  features.forEach((feature) => {
    const currentIndex = indexes[feature.geometry.type];
  ...
  customSelectFeatureId: async (
    ...
    if (deselect) {
      ...
      if (state.selectedFeatureIds.length > 0) {
        const lastItemIndex = state.selectedFeatureIds.reduce((result, item) => {
